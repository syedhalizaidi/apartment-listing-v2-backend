...
# ── [ADD NEAR TOP-LEVEL / WITH OTHER CONSTANTS] ────────────────────────────────
LEOPALACE_SUPPORT_NUMBER = "+81 50-3652-6379"
VILLAGEHOUSE_SUPPORT_NUMBER = "0120-266-078"


# ── [ADD THIS HELPER ANYWHERE NEAR OTHER HELPERS] ──────────────────────────────
def _detect_provider_brand(item: Dict[str, Any]) -> Optional[str]:
    """
    Try to detect listing provider/brand from common fields.
    Returns one of: 'leopalace', 'villagehouse', or None.
    """
    if not isinstance(item, dict):
        return None

    # Collect text haystack
    texts = []
    for k in ("brand", "provider", "source_name", "source", "title", "url", "details_url"):
        v = item.get(k)
        if isinstance(v, str) and v:
            texts.append(v)
    # also peek into document/description if available
    for k in ("description", "document", "text"):
        v = item.get(k)
        if isinstance(v, str) and v:
            texts.append(v)
    ...
    # Normalize a little (common spellings)
    hay = hay.replace("leo palace", "leopalace").replace("village house", "villagehouse")

    # Domain hints
    if "leopalace" in hay or "leopalace21" in hay or "leopalace.jp" in hay:
        return "leopalace"
    if "villagehouse" in hay or "villagehouse.jp" in hay:
        return "villagehouse"

    return None


def _pick_contact_number_from_chosen(chosen: List[Dict[str, Any]]) -> Optional[str]:
    """
    Priority:
      1) If any selected item is Leopalace → fixed LEOPALACE_SUPPORT_NUMBER
      2) Else if any selected item is Village House → fixed VILLAGEHOUSE_SUPPORT_NUMBER
      3) Else first non-empty item['contact_phone'] (as-is)
      4) Else None
    """
    if not chosen:
        return None
    ...
    # 1) Leopalace?
    for it in chosen:
        if _detect_provider_brand(it) == "leopalace":
            return LEOPALACE_SUPPORT_NUMBER

    # 2) Village House?
    for it in chosen:
        if _detect_provider_brand(it) == "villagehouse":
            return VILLAGEHOUSE_SUPPORT_NUMBER

    # 3) item-provided contact_phone (first non-empty)
    for it in chosen:
        cp = (it.get("contact_phone") or "").strip()
        if cp:
            return cp

    return None
# ── [REPLACE YOUR EXISTING handle_contact WITH THIS VERSION] ───────────────────
def handle_contact(user_id: str, message: str, settings: Optional[BotSettings] = None, auto_followup: bool = False) -> str:
    ...
    After full contact, returns a brand-specific contact number:
      - Leopalace  -> +81 50-3652-6379
      - VillageHouse -> 0120-266-078
      - else: listing.contact_phone (if present)
    """
    settings = settings or BotSettings()
    sess = ensure_session(user_id)
    ...
    updated = False
    if extracted["name"] and extracted["name"].strip() and len(extracted["name"].split()) >= 1 and extracted["name"].lower() not in ["hi", "hello", "hey"]:
    ...
    if extracted["email"] and "@" in extracted["email"] and "." in extracted["email"].split("@",1)[1]:
        ...
        updated = True
    if extracted["phone"] and re.match(r"^\d{7,15}$", extracted["phone"]):
    ...
    if not missing:
        ...
        # ✅ brand-based contact number selection
        contact_number = _pick_contact_number_from_chosen(chosen)

        if not chosen:
            brief = {
                "situation": "contact_complete_no_choice",
                "fallback": (
                    "All set with your info! Ready to hunt more or book something?"
                    + (f"\n\nIf you need to reach us directly, use: {contact_number}" if contact_number else "")
                ),
            }
        ...
        brief = {
            ...
            "time": time_str,
            "contact": contact,
            "contact_number": contact_number,  # << include for your reply composer
            "fallback": (
                f"Locked in! You're eyeing:\n{summary}\n\n"
                f"Time: {time_str}\n"
                f"You: {contact['name']} | {contact['email']} | {contact['phone']}"
                f"{number_line}\n\n"
